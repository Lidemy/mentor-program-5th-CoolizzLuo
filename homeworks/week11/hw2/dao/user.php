<?php
  ...
  function getUserByUsername($username) {
    global $conn;
    $sql = 'SELECT * FROM enzo_blog_users WHERE username = ?';
  ...
  function getAllUser() {
    ...
    $stmt = $conn->prepare('SELECT * FROM enzo_blog_users');
    $result = $stmt->execute();
    if (!$result) {
      die('Error: ' . $conn->error);
    }
  ...
  function addUser($nickname, $username, $password) {
    ...
    $sql = 'INSERT INTO enzo_blog_users(nickname, username, password) values(?, ?, ?)';
    $stmt = $conn->prepare($sql);
  ...
  function updateUser($nickname, $username) {
    ...
    $sql = 'UPDATE enzo_blog_users SET nickname=? WHERE username=?';
    $stmt = $conn->prepare($sql);
  ...
  function updateRoleById($role, $id) {
    ...
    $sql = 'UPDATE enzo_blog_users SET role=? WHERE id=?';
    $stmt = $conn->prepare($sql);
